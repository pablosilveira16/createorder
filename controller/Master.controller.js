                    ...
                    oDataModel.read("/NotifSet?$expand=NotifItemSet,NotifActivitySet,NotifLongtextSet,NotifTaskSet,NotifPhotoSet", {
                        success: function(oData, response) {
                            var aNotifications = oData.results,
                              aEquipments = localModel.getProperty("/Equipments").map(function(e) { return e.Equipment; }),
                              aActualNotifications = [];
                            for (var i in aNotifications) {
                              if (aEquipments.indexOf(aNotifications[i].Equipment) !== -1) {
                                aActualNotifications.push(aNotifications[i]);
                              }
                            }
                            localModel.setProperty("/Notifications", aActualNotifications, localModel, true);
                            localModel.setProperty("/ActualNotifications", aActualNotifications, localModel, true);
                            dfdNotifSet.resolve();
                        },
                        error: function(oError) {
                            dfdNotifSet.reject();
